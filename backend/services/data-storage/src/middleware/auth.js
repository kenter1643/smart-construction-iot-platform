...
function requireScopedDeviceIdInQuery(key = 'deviceId') {
  return requireDeviceScope((req) => req.query?.[key]);
}

function requireScopedDeviceIdInBody(key = 'deviceId') {
  return requireDeviceScope((req) => req.body?.[key]);
}
