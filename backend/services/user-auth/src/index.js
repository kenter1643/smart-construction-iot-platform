...
const { initMySQL, testConnections, initAuthTables } = require('./utils/db');
...
const startServer = async () => {
  try {
    ...
    httpServer.listen(PORT, () => {
      console.log(`User Authentication Service (HTTP) listening on port ${PORT}`);
      console.log(`Health check: http://localhost:${PORT}/health`);
      console.log(`API documentation: http://localhost:${PORT}/api/v1/auth`);
    });
    ...
    if (HTTPS_ENABLED && SSL_KEY_PATH && SSL_CERT_PATH) {
      try {
        ...
        httpsServer.listen(HTTPS_PORT, () => {
          console.log(`User Authentication Service (HTTPS) listening on port ${HTTPS_PORT}`);
          console.log(`Health check: https://localhost:${HTTPS_PORT}/health`);
          console.log(`API documentation: https://localhost:${HTTPS_PORT}/api/v1/auth`);
        });
      } catch (sslError) {
